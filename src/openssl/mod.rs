...
mod test {
    ...
    /// Test vector for `SECP256K1-SHA256-SVDW` cipher suite
    /// ASCII: "sample"
    /// Oracle source: Generated by direct sage implementation https://github.com/mimblewimble/secp256k1-zkp/blob/master/sage/shallue_van_de_woestijne.sage
    #[test]
    fn test_hash_to_point_svdw_1() {
        ...
        let public_key_point = vrf.derive_public_key_point(&secret_key).unwrap();

        // Data to be hashed with svdw (ASCII "sample")
        let data = hex::decode("73616d706c65").unwrap();
        let hashed_point = vrf.hash_to_point_svdw(&public_key_point, &data).unwrap();
        let expected_vector = BigNum::from_hex_str(
            "04bf6b0a69e608da4d9f9f1be84f5f7fd92e097f4467993a77758c4d603be1ba73a0175afbae1083465d6adf240802e8a65b63e330de60941ae513ffffd3d21740"
        ).unwrap().to_vec();
        assert_eq!(expected_vector.len(), 65);
        let expected_point =
            EcPoint::from_bytes(&vrf.group, &expected_vector, &mut vrf.bn_ctx).unwrap();
        assert!(hashed_point
            .eq(&vrf.group, &expected_point, &mut vrf.bn_ctx)
            .unwrap());
    }

    /// Test vector for `SECP256K1-SHA256-SVDW` cipher suite
    /// ASCII: "some long message to verify correctness of svdw implementation"
    /// Oracle source: Generated by direct sage implementation https://github.com/mimblewimble/secp256k1-zkp/blob/master/sage/shallue_van_de_woestijne.sage
    /// But the oracle source does not follow the standardized sign0 method discussed in https://tools.ietf.org/html/draft-irtf-cfrg-hash-to-curve-04.
    /// The oracle is determining the sign of y coordinates based on parity. It is a matter of choice, so the sage code has been changed to match with the standard
    /// and then the oracle was generated.
    #[test]
    fn test_hash_to_point_svdw_2() {
        let mut vrf = ECVRF::from_suite(CipherSuite::SECP256K1_SHA256_SVDW).unwrap();
        ...
        // Data to be hashed with svdw (ASCII "some long message to verify correctness of svdw implementation")
        let data = hex::decode("736f6d65206c6f6e67206d65737361676520746f2076657269667920636f72726563746e657373206f66207376647720696d706c656d656e746174696f6e").unwrap();
        let hashed_point = vrf.hash_to_point_svdw(&public_key_point, &data).unwrap();
        let expected_vector = BigNum::from_hex_str(
            "04665614e0f145bfa39bfb3cfa5a7db213cc7d6f486f03639f7b55503383dba96abdadd77012ef1d5f67fef9bbdfd58d5da53cd2c8e38a063319c4dc7b5d4bd46b"
        ).unwrap().to_vec();
        assert_eq!(expected_vector.len(), 65);
        let expected_point =
            EcPoint::from_bytes(&vrf.group, &expected_vector, &mut vrf.bn_ctx).unwrap();
